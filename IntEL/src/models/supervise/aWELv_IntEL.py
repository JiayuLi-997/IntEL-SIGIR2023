'''
Utilize model structure of IntEL for aWELv-like loss
'''
...
class aWELv_IntEL(GeneralSeq.GeneralSeq):
	reader="SeqReader"
	...
	def __init__(self,args,corpus):
		...
		self.pred_layer = nn.Linear(
					...
					args.context_emb_size+args.u_emb_size,#+args.intent_emb_size+args.intent_emb_size
					self.intent_num)
	...
	def predict_ensemble(self,data,intent):
		...
		# # to weight
		h_intent = F.relu(self.intent_embeddings(h_int)).squeeze()
		all_xatt = torch.cat([item_xatt.mean(dim=1),score_xatt.mean(dim=1),h_u,h_intent],dim=-1)
		weights_item = self.weight_embeddings(all_xatt)#.softmax(dim=-1)
		weights = weights_item.unsqueeze(dim=1).repeat(1,h_i.size(1),1).softmax(dim=-1)
